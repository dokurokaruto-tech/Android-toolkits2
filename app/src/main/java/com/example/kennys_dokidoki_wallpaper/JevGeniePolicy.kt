...
/** カタログのタグ1件分のスナップショット */
internal data class JevGenieTagRef(val name: String, val text: String)
...
internal object JevGeniePolicy {
    ...
    fun systemPrompt(cards: List<JevGenieCardRef>, tags: List<JevGenieTagRef>): String = buildString {
        ...
        appendLine("- $TOOL_TAG: タグ文章の編集。targetはタグ名。after.$FIELD_TEXT 必須（日本語）。")
        ...
        tags.forEach { appendLine("- ${it.name} :: ${clip(it.text)}") }
    }
    ...
    private fun tagPlan(json: JSONObject, tags: List<JevGenieTagRef>, reply: String): JevGeniePlan.EditTag {
        val target = pick(json.optString("target").trim(), tags, { it.name }, "タグ")
        val after = json.optJSONObject("after") ?: JSONObject()
        val newText = text(after, FIELD_TEXT, "変更後のtext")
        return JevGeniePlan.EditTag(target, newText, reply.ifEmpty { "タグ『${target.name}』を更新します。" })
    }
