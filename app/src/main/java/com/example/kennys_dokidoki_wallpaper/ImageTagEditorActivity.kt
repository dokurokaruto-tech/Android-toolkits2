...
class ImageTagEditorActivity : AppCompatActivity() {
    ...
    private fun setupUI() {
        ...
        btnSave.setOnClickListener {
            if (isBatchMode) {
                ...
                urisToEdit.forEach { uriStr ->
                    val entry = DataManager.findImageByUri(uriStr) ?: return@forEach
                ...
                val liveEntry = DataManager.findImageByUri(imageEntry?.uri?.toString()) ?: imageEntry
                if (liveEntry == null) {
                    Toast.makeText(this, "画像データが見つからないから保存できなかったわ。", Toast.LENGTH_LONG).show()
                    return@setOnClickListener
                }
                val tagsToAdd = selectedTags - initialEffectiveTags
                TagManager.autoRegisterUniformImpliedTags(this, liveEntry.tags, tagsToAdd)
    ...
    private fun showDirectDescriptionDialog(entry: ImageEntry) {
        ...
        btnSave?.setOnClickListener {
            val liveEntry = DataManager.findImageByUri(entry.uri.toString()) ?: entry
            val originalMeta = TavernCardParser.getTavernMeta(liveEntry.description)
            val newText = etDesc?.text?.toString()?.trim() ?: ""
            if (newText.isEmpty()) {
                liveEntry.description = null
            } else {
                if (originalMeta != null) {
                    liveEntry.description = "$newText\n\n<TavernMeta>\n$originalMeta\n</TavernMeta>"
                } else {
                    liveEntry.description = newText
                }
            }
    ...
    private fun generateDescriptionAi(entry: ImageEntry, targetEditText: EditText, statusTextView: TextView, systemPrompt: String) {
        ...
        statusTextView.setTextColor(android.graphics.Color.parseColor("#FFCC00"))
            ...
            try {
                ...
                if (reply.isNotEmpty()) {
                    withContext(Dispatchers.Main) { 
                        ...
                        statusTextView.setTextColor(android.graphics.Color.parseColor("#00FF00"))
                ...
                withContext(Dispatchers.Main) { 
                    ...
                    statusTextView.setTextColor(android.graphics.Color.parseColor("#F2B8B5"))
