...
/**
 * プロンプトカードのAI画面の文言。タグ側と同じ並びにし、中身だけカード用にする。
 */
object PromptCardAiCopy {
    ...
    const val USE_CARD_LABEL = "カード名を含める"
    const val USE_EXISTING = "今のプロンプトを下書きにする"
    ...
    const val NEED_INPUT = "補足か、いまのプロンプトを指定してほしい。"
    const val DONE = "置き換えた。"
    const val FAILED = "置き換えに失敗した。"

    fun modelLine(choice: PromptCardLlmChoice): String = choice.displayName
}
