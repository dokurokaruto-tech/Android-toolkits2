...
テストはGPU不要です。`imageio-ffmpeg` 導入時はAAC/ALACのM4Aを実際に作成・変換するテストも実行します。実機では「初回だけ音声POST」「PC再起動後の再利用」「タグ名変更後の再利用」「旧音声の個別削除」「全件削除後の再送」に加え、「単一タグで再生」「2タグで拒否」「継承タグの競合」「生成後の画像変更」「画面終了時の停止」「Forge生成中の拒否」を確認してください。
