...
3. **ライブラリが不足／指定バージョンと不一致**：CUDA版PyTorch・torchaudio・Qwen TTS・FFmpeg・Pillow等の導入を確認。数GBのダウンロードが発生する場合があります。
...
アプリとPC側を両方更新してください。PCで `setup-tts.bat` を再実行し、ライブラリ導入確認に **Y** を選び、チェック後にエージェントを再起動します。
追加の `imageio-ffmpeg==0.6.0` がFFmpeg実行ファイルを供給します。WindowsにFFmpegを別途入れる必要はありません。既存のモデルパスは変更不要です。
...
RTX 2070向けに **FP16 + SDPA** を使用します。BF16・FlashAttention 2は要求しません。初期設定では1回ごとに子プロセスでモデルをロードし、終了時にVRAMを解放します。下記の高速モードでは待機中に再利用します。長文は約200文字ずつ順番に処理します。

`tts_unload_sd: true` では、Forgeに接続できる場合、TTS前にチェックポイントをアンロードし、終了後に再ロードします。アンロードAPI非対応ならエラーになります。復帰APIの404は警告に留めます。Forgeを停止するか、手動でGPUを空けてから `false` にしてください。接続できない場合はTTS単独で動作します。
...
**更新だけでは有効になりません。** 既存の動作を維持するため、初期設定は標準・毎回終了です。

1. PC側を更新し、エージェントと他のGPUアプリを停止します。
2. **`setup-tts-fast.bat`** を実行し、導入・設定変更の確認で **Y**。
3. `start-agent.bat` を起動し、短い返信を2回続けて読み上げます。Android側の更新は不要です。

モデルの再ダウンロード・名前変更はしません。追加するのは `faster-qwen3-tts==0.3.2` と互換依存関係です。
既存のCUDA PyTorch 2.7.1 / Qwen 0.1.1を維持し、Transformersは4.57.3に固定します。
0.4系はQwen配布物・Transformers世代が変わるため、自動更新しません。
Nならインストール・設定変更は行いません。チェック失敗時は高速設定へ切り替えませんが、承認済みのパッケージ導入は残ります。

高速モードでは次を行います。

- **CUDA Graph**でトークン生成ループの呼び出し負荷を削減。FP16 + SDPAを維持し、FlashAttention・Triton・別モデルを導入しません。
- **モデル・CUDA Graphを最大120秒の待機中保持**。連続TTSでは読み込み・初期化を繰り返しません。
- 音声SHA256と文字起こしをキーに、最大2件のクローン用特徴をプロセス内で再利用。別音声・別文字起こしへの置き換えはしません。
- エージェントの画像生成・チェックポイント切替の前にTTSプロセスを終了し、VRAMを解放します。SD状態取得・中断APIは塞ぎません。
- 待機時間超過、生成失敗、タイムアウト、正常なエージェント終了時にも解放します。音声削除時は特徴キャッシュも破棄します（実行中なら完了後）。

**Forge画面での直接操作や、別アプリのGPU使用は制御できません。** 高速モードでTTSと同時に操作しないでください。
TTS解放時にSDが復帰すると、VRAM全体の使用量はゼロにはなりません。
エージェントを強制終了した場合もワーカーには待機・生成の自己終了制限がありますが、一時ファイルが残る場合があります。

| `config.local.json` | 意味 |
|---|---|
| `tts_backend: "standard"` | 従来のQwen実装 |
| `tts_backend: "cuda_graph"` | 指定版の高速実装（追加導入が必要） |
| `tts_keep_alive_seconds: 120` | GPU上に保持する待機秒数。0で毎回終了、上限600 |

モデル保持だけ試す場合は `standard` と `tts_keep_alive_seconds: 120` を組み合わせられます。追加パッケージは不要です。
設定変更後はエージェントを再起動してください。標準モードでもモデル保持中は同じVRAM制約があります。

**戻す場合：`setup-tts-standard.bat` → Y → エージェント再起動。** 標準実装・保持0秒に戻します。モデル・保存音声・APIキーを削除せず、パッケージもアンインストールしません。
CUDA GraphのVRAM不足や音質問題が出た場合はこちらを使ってください。失敗したTTSを別方式で黙って再生成することはありません。

初回はモデル読込とGraph準備が必要です。120秒以上空いた後や画像生成後も初回扱いになります。
**RTX 2070 / Windowsでの速度・VRAM使用量・音質は未測定**です。高速化の倍率やリアルタイム生成は保証しません。
...
- `model_reused` / `prompt_cache`：2回目に `true` / `hit` なら再利用できています。
- `imports_s` / `model_s`：ライブラリとモデルの読込時間。
- `reference_s`：サンプル変換・話者特徴抽出時間。
- `generation_s`：本文の生成・音声デコード時間。初回はGraph準備も含みます。
- `worker_total_s` / `audio_s`：ワーカー処理時間／生成された音声の長さ（付加無音を除く）。
- `seconds_per_audio_second`：処理時間 ÷ 音声長。小さいほど速く、1未満ならワーカー処理が音声長より短い値です。
- `sd_unload` / `sd_restore` / `request_total`：SD切替とPC要求全体。ネットワーク転送・Android再生待ちは含みません。

GPU使用率100%だけでは、モデル読込・GPU演算・切替のどこが支配的かは分かりません。
同じ本文・サンプルで初回と2回目を比べてください。タイミングログに本文・原音声・APIキーは出しません。

`reload-checkpoint` の **404** はSD側に復帰APIがないことを示します。音声生成失敗とは区別し、一度警告した後は同APIを繰り返し呼びません。
次の画像生成でSD側がモデルをロードする必要があります。ロードされなければForge側でモデルを選び直してください。
404以外の通信・サーバーエラーは引き続き記録します。404だけを根拠に、それまでの生成時間の原因とは判断しません。

#### 調査した実装

- [FlashAttention公式のGPU要件](https://github.com/Dao-AILab/flash-attention#nvidia-cuda-support)：通常のFlashAttention 2はAmpere以降。RTX 2070向けにそのまま導入する方法は採用しません。
- Faster Qwen3-TTS [5](https://github.com/andimarafioti/faster-qwen3-tts)：CUDA Graph・SDPA方式、WindowsおよびTuring世代T4の報告があります。ただしRTX 2070の速度保証ではありません。
- [採用版0.3.2](https://pypi.org/project/faster-qwen3-tts/0.3.2/)：Qwen 0.1.1 / Transformers 4系との依存関係と、クローン用特徴の再利用APIを確認しました。
...
- サンプル：WAV / FLAC / MP3 / M4A（AAC・ALAC）、1〜30秒（3秒以上推奨）、6 MiB以下。PCで24kHzモノラルWAVへ変換します。利用許可のある、単一話者の明瞭な音声を使ってください。
- 本文：2,000文字以下。返信候補・画像タグ・思考タグは読み上げから除きます。
- サンプルはアプリ専用領域へコピーするため、選択元のファイルを移動しても使用できます。PCに同じタグID・音声SHA256があるか毎回確認し、未保存時だけ送ります。保存済みなら端末の音声ファイルも読みません。別PC・削除後は再送します。
- 生成時の音声付きタグが **0個なら未設定エラー、2個以上なら競合エラー**。同じ音声ファイルでもタグが2つなら拒否します。継承された「自動的に持つタグ」も対象です。
- 返信には生成時の音声設定を保存します。後からタグ・画像・音声を変更しても過去の返信は別の声になりません。変更を適用するには返信を再生成してください。音声情報のない旧履歴は再生成が必要です。タグIDだけがない旧履歴は現存タグから補完し、音声SHA256は変更しません。
- 過去の返信を保護するため、音声の解除・差し替えで保存済みサンプルは削除しません。JSONバックアップには紐付けのみ含み、音声本体は含みません。タグIDもバックアップに含みます。同じPCに保存済みなら別端末でも再利用できます。PCにも端末にも実体がない音声は再選択と返信再生成が必要です。
- 画面を離れると通信・再生を停止します。生成中のアイコンでも通信を中止できますが、PCの処理は完了／制限時間まで続く場合があります。TTSは永続キューではなく、失敗時は手動で再試行します。
- **アップロード済みサンプルはPCに残ります。** 変換用の一時コピーは要求終了時に削除し、Androidの生成音声は再生終了／停止時に削除します。異常終了時の一時ファイルは残る場合があります。
- Tailscale等の信頼できるネットワーク内だけで利用し、`api_key` を設定してください。ポートをインターネットへ直接公開しないでください。

### PCの保存音声を削除

タグ編集 → **PC保存音声を管理** → 音声を選ぶ（または全件削除）→ 確認。
音声設定を保存してから編集画面を開き直すと管理できます。差し替え前の版も一覧に残り、タグ名変更・音声の紐付け解除では消えません。

- 削除は即時適用です。タグ編集をキャンセルしても戻りません。
- 端末側のコピーは削除しません。過去の返信も含め、次回TTSで必要なら再送します。
- 実行中の生成は止まりません。削除前に開始したアップロードは状態IDで拒否します。
- **タグ自体を削除する前にPC音声を削除してください。** タグ削除ではPCへ接続せず、管理用IDが端末からなくなります。
- PC保存先は `database_path` と同じフォルダーの `tts-voices.sqlite3`（標準 `data/tts-voices.sqlite3`）。原音声・登録時のタグ名・ファイル名を保存します。暗号化はしません。
- 上限は全音声合計512 MiB、1タグ256版。超過時は不要な版を削除してください。自動削除で過去の声を置き換えることはありません。
- SQLiteの削除領域は上書きしますが、OS・ディスク・バックアップ上の完全消去は保証しません。全タグの保存音声を消す場合はエージェント停止後にこのDBを削除できます（次回TTSで再送されます）。

### APIと確認

すべて既存のBearer認証を使います。`tag_id` は32桁の小文字16進ID、`sample_id` は元音声のSHA256（64桁）です。

- `GET /api/v1/tts/voices/<tag_id>` → `{tag_id, epoch, samples}`。一覧はID・名前・作成時刻・サイズのみで、音声本体は返しません。
- `POST /api/v1/tts/voices/<tag_id>` → `{epoch, sample_id, tag_name, name, audio_base64}` を登録。同じ組み合わせの再登録は重複保存しません。
- `DELETE /api/v1/tts/voices/<tag_id>?epoch=<epoch>&sample_id=<sample_id>` → 指定版を削除。`sample_id` 省略でタグ全件削除。`{deleted, epoch}` を返します。
- `epoch` はGETの値を使用。削除後に変わり、古い値の書き込み・削除は409になります。
...
{
  "text": "こんにちは。",
  "voices": [{"tag_id": "32桁のタグID", "sample_id": "64桁の音声SHA256", "ref_text": "サンプルの文字起こし（省略可）"}]
}
```

成功は `audio/wav`、エラーは `{"error":"理由"}`。400: 入力不正、401: 認証、404: 保存音声なし、409: GPU使用中、503: モデル未設定、500: 生成失敗。旧形式の `audio_base64` 直接指定も受け付けますが、タグに紐付けず一時処理のみです。保存IDとの併用は不可です。サーバーも `voices` が1個であることを検証します。`GET /api/v1/health` の `tts_voice_cache: true` が保存API対応を示します。`tts_configured` はパスの設定有無であり、モデルの動作保証ではありません。

```bat
python -m unittest discover -s tests -v
```

テストはGPU不要です。高速化では実サブプロセスを使うIPC・再利用・終了テストと、モデルを模擬した特徴キャッシュ・FP16設定テストを含みます（実GPUの速度試験ではありません）。`numpy` / `soundfile` がない環境ではエンジンの模擬テストをスキップします。`imageio-ffmpeg` 導入時はAAC/ALACのM4Aを実際に作成・変換するテストも実行します。実機では「初回だけ音声POST」「PC再起動後の再利用」「タグ名変更後の再利用」「旧音声の個別削除」「全件削除後の再送」に加え、「単一タグで再生」「2タグで拒否」「継承タグの競合」「生成後の画像変更」「画面終了時の停止」「Forge生成中の拒否」を確認してください。
